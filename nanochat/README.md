...
**Training Status**: This implementation has completed the full training pipeline (tokenization → base pretraining → midtraining → supervised fine-tuning) for a d20 model (561M parameters) at a total cost of ~$95. See [nanochat_project_plan.md](nanochat_project_plan.md) for complete results and benchmarks.

**TinyChat (Phase 2)**: The model has been customized as "TinyChat" — a portfolio project by Jonathan Avni. Identity data was generated via synthetic conversations and mixed into SFT training at ~17% of the dataset. The model is deployed live with a Next.js frontend on Vercel backed by Modal (T4 GPU, serverless). **Try it: https://tinychat-two.vercel.app**
